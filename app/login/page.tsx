...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    // ── Admin special case ──
    // If no profile row exists OR role is admin → go to admin directly
    if (profileErr || !profile) {
      // Profile doesn't exist — could be an admin created directly in Supabase Auth
      // Check email pattern or just redirect to admin
    ...
    // ── Status checks for non-admin ──
    if (profile.status === 'pending')  { setMode('pending'); return }
    if (profile.status === 'rejected') { setErr('Compte rejeté. Contactez le support.'); return }
    if (profile.status === 'banned')   { setErr('Compte suspendu.'); return }

    showToast('Connexion réussie !', 'success')
    if (profile.role === 'restaurant') router.push('/restaurant')
  ...
  const handleRegisterCustomer = async () => {
    ...
    await supabase.from('profiles').upsert({
      ...
      address, id_card_url: idUrl, status: 'pending',
    })
  ...
  const handleRegisterRestaurant = async () => {
    ...
    if (!idDocFile || !ownershipFile) { setErr('CIN + justificatif de propriété requis'); return }
    if (pass.length < 6) { setErr('Mot de passe minimum 6 caractères'); return }
    setErr(''); setLoading(true)
    ...
    const [logoUrl, idUrl, ownerUrl] = await Promise.all([
      logoFile ? uploadFile(logoFile, 'restaurants', `${uid}/logo.${logoFile.name.split('.').pop()}`) : undefined,
    ...
    await supabase.from('profiles').upsert({
      ...
      restaurant_name: restName, restaurant_description: restDesc,
      restaurant_address: restAddress, restaurant_phone: restPhone,
      restaurant_cuisine: restCuisine.split(',').map(s => s.trim()).filter(Boolean),
      restaurant_logo_url: logoUrl, id_doc_url: idUrl, ownership_doc_url: ownerUrl,
      status: 'pending',
    })
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-8">
          ...
          <div className="w-full max-w-md page-enter">
            <div className="text-center mb-8">
              ...
              <h1 className="font-display text-4xl font-bold leading-tight">
                Accédez à votre<br /><span className="text-savora-orange">espace</span>
              </h1>
            </div>
            <div className="glass rounded-2xl p-6 space-y-3">
              <button onClick={() => setMode('login')} className="btn-primary w-full text-sm py-3.5">
                Se connecter
              </button>
              <button onClick={() => setMode('register-role')} className="btn-ghost w-full text-sm py-3.5">
                Créer un compte
              </button>
            </div>
          </div>
        ...
        {mode === 'login' && (
          <div className="w-full max-w-md page-enter">
            ...
            <div className="glass rounded-2xl p-6 space-y-4">
              ...
              <button onClick={handleLogin} disabled={loading} className="btn-primary w-full py-3.5 mt-2">
        ...
        {mode === 'register-role' && (
          <div className="w-full max-w-md page-enter">
            ...
            <div className="space-y-3">
              <button onClick={() => setMode('register-customer')}
                className="glass w-full rounded-2xl p-5 text-left hover:border-savora-orange/30 transition-all group card-lift">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 rounded-xl bg-savora-orange/10 flex items-center justify-center">
                  ...
                  <span className="text-white/20 group-hover:text-savora-orange transition-colors">→</span>
                </div>
              ...
              <button onClick={() => setMode('register-restaurant')}
        ...
        {mode === 'register-customer' && (
          <div className="w-full max-w-lg page-enter">
            ...
            <p className="text-sm text-white/45 mb-6">Votre compte sera vérifié par l&#39;admin avant activation</p>
            ...
            <div className="glass rounded-2xl p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-white/50 mb-1.5">Carte d&#39;identité nationale (CIN) *</label>
              ...
              <button onClick={handleRegisterCustomer} disabled={loading} className="btn-primary w-full py-3.5 mt-2">
        ...
        {mode === 'register-restaurant' && (
          <div className="w-full max-w-2xl page-enter">
            ...
            <div className="glass rounded-2xl p-6 space-y-5">
              <div>
                <p className="text-xs font-bold text-white/40 uppercase tracking-wider mb-3">Propriétaire</p>
                <div className="grid grid-cols-2 gap-3">
                  {[
              ...
              <div>
                <p className="text-xs font-bold text-white/40 uppercase tracking-wider mb-3">Restaurant</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-white/50 mb-1.5">Cuisine</label>
                    <div className="field-wrap flex items-center gap-2 rounded-xl border border-white/[0.07] bg-black/40 px-3 py-2.5">
                ...
                <div className="mt-3">
                  ...
                  <div className="field-wrap flex items-center gap-2 rounded-xl border border-white/[0.07] bg-black/40 px-3 py-2.5">
                    ...
                    <input value={restAddress} onChange={e => setRestAddress(e.target.value)} className="w-full bg-transparent text-sm placeholder:text-white/25" placeholder="Adresse complète" />
                  </div>
                </div>
                <div className="mt-3">
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  ...
                  <FileBtn label="Justificatif de propriété * (patente, bail...)" file={ownershipFile} onClick={() => ownershipRef.current?.click()} icon={ShieldCheck} />
                </div>
              </div>

              <button onClick={handleRegisterRestaurant} disabled={loading} className="btn-primary w-full py-3.5">
          ...
          <div className="w-full max-w-md text-center page-enter">
            <div className="glass rounded-2xl p-8">
              ...
              <p className="mt-3 text-sm text-white/50 leading-relaxed">
                Votre dossier est en cours d&#39;examen. Vous serez notifié par email une fois approuvé.
              </p>
